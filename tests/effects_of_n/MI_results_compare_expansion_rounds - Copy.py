...
stop_compare_medians_concat_diff = stop_compare_medians_concat.diff().iloc[1:].ge(1).astype(int)
stop_compare_medians_concat_diff_cumsum = stop_compare_medians_concat_diff.cumsum()

stop_compare_medians_concat_diff_cumsum_converged = pd.DataFrame(index=stop_compare_medians_concat.index, columns=stop_compare_medians_concat.columns)
for stop in [0] + stop_list:
    stop_compare_medians_concat_diff_cumsum_converged.loc[stop] = stop_compare_medians_concat_diff_cumsum.loc[stop_list[-1]] == stop_compare_medians_concat_diff_cumsum.loc[stop]

stop_compare_medians_concat_diff_cumsum_converged_count = stop_compare_medians_concat_diff_cumsum_converged.sum(axis=1)
stop_compare_medians_concat_diff_cumsum_converged_count.plot(kind='line')
